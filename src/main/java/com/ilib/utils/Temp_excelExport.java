...
public class Temp_excelExport {

	

	/**
	 * Excel 相关操作类(大数据量写入但受Excel数据行数限制 可写100多W条数据)
	 ...
	 */
	     ...
	     * @param objects   数据信息
	     */
	    public void writeExcelData(String directory, String fileName, String sheetName, List<?> objects,String[] valueproperies)
	            throws Exception, IOException {
	        File tmpFile = assertFile(directory, fileName);
	        outputStream = new FileOutputStream(tmpFile);
	        exportExcelData(sheetName, objects,valueproperies);
	    }
	     ...
	     * @param objects 目标数据
	     */
	    private void exportExcelData(String sheetName, List<?> objects,String[] valueproperies) throws Exception, IOException {
	        // 正文样式
	        ...
	        for (Object dataRow : objects) {
	            Row row = sheet.createRow(currentRowNum);
	            for (int j = 0;j<valueproperies.length;j++) {
	            	String property = valueproperies[j];
	        	    Cell contentCell = row.createCell(j);
					Object dataObject = getFieldValueByName(property, dataRow);
					 if (dataObject != null) {
	    ...
	    public void output(String sheetTitle, LinkedList<String> columnNames,String[] valueproperies,String path,String fileName, List<?> list)throws Exception{
	    	String sheetName = sheetTitle;
	        writeExcelTitle(path, fileName, sheetName, columnNames, sheetTitle);
	        try {
	            writeExcelData(path, fileName, sheetName, list,valueproperies);
	        } catch (Exception e) {
	            e.printStackTrace();
	        }finally{
	        	 list.clear();
	    ...
	    public static void main(String[] args)throws Exception {
	    	 Temp_excelExport exportExcel2007 = new Temp_excelExport();
	    	 LinkedList<String> columnNames = new LinkedList<>();
	        columnNames.add("起始");
	        //columnNames.add("抵达");
	        columnNames.add("起飞日期");
	        //columnNames.add("票面价");
	        String[] valueproperies = new String[]{"dep","date"};
	        String path = "d:/tmp";
	        ...
	        exportExcel2007.output("test", columnNames, valueproperies, path, fileName, dataA);
		}
	}
	class testBean{
		private String dep;
